...
import de.blinkt.openvpnsc.core.*;
import ru.rutoken.pkcs11.session.SessionWrapperManager;
import ru.rutoken.pkcs11.session.SessionWrapper;
import ru.rutoken.utility.CertificateConverter;
import ru.rutoken.businessrules.RsaSigner;
...
public class VpnProfile implements Serializable, Cloneable {
    ...
    public static final String EXTRA_PROFILEUUID = "de.blinkt.openvpnsc.profileUUID";
    ...
    public static final int TYPE_TOKEN_CERTIFICATES = 8;
    ...
    /* pkcs11 members */
    public String mTokenSerial;
    public X509Certificate mTokenAuthCert;
    ...
    public String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        switch (mAuthenticationType) {
            ...
            case VpnProfile.TYPE_USERPASS:
                ...
                break;
            case TYPE_TOKEN_CERTIFICATES:
                cfg += insertFileData("ca", mCaFilename);
                try {
                    cfg += "<cert>\n" + CertificateConverter.X509CertificateToPEMString(mTokenAuthCert) + "\n</cert>\n";
                }
                catch (Exception e) {
                    e.printStackTrace();
                    return null;
...
//        }
    ...
    public int needUserPWInput(String transientCertOrPkcs12PW, String mTransientAuthPW, String transientTokenPin) {
        if ((mAuthenticationType == TYPE_PKCS12 || mAuthenticationType == TYPE_USERPASS_PKCS12) &&
        ...
        if (mAuthenticationType == TYPE_TOKEN_CERTIFICATES) {
            if (transientTokenPin == null)
                return R.string.token_pin;
        }
    ...
    public String getTokenPin() {
        return PasswordCache.getTokenPin(mUuid, true);
    }
    ...
    public String getSignedData(String b64data) {
        if (mAuthenticationType == TYPE_TOKEN_CERTIFICATES) {
            if (mTokenSerial == null) {
                VpnStatus.logError(R.string.no_token_in_profile);
                return null;
            }
            if (mTokenAuthCert == null) {
                VpnStatus.logError(R.string.no_cert_in_profile);
                return null;
            }

            byte[] decodedData = Base64.decode(b64data, Base64.DEFAULT);

            try {
                SessionWrapperManager sessionManager = SessionWrapperManager.getInstance();
                Optional<Pkcs11Token> token = sessionManager.hasSessionWrapper(mTokenSerial) ?
                        Optional.of(sessionManager.getSessionWrapper(mTokenSerial).getSession().getToken()) :
                        TokenFinder.getBySerial(mTokenSerial);
                try (SessionWrapper session = new SessionWrapper.Builder(token.get()).setPin(getTokenPin()).build()) {
                    byte[] signedBytes = new RsaSigner(session, mTokenAuthCert).signData(decodedData);
