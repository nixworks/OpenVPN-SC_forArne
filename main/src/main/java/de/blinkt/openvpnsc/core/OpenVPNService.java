...
import de.blinkt.openvpnsc.LaunchVPN;
import de.blinkt.openvpnsc.R;
import de.blinkt.openvpnsc.VpnProfile;
import de.blinkt.openvpnsc.activities.DisconnectVPN;
import de.blinkt.openvpnsc.activities.MainActivity;
import de.blinkt.openvpnsc.api.ExternalAppDatabase;
import de.blinkt.openvpnsc.core.VpnStatus.ByteCountListener;
import de.blinkt.openvpnsc.core.VpnStatus.StateListener;
...
import ru.rutoken.pkcs11.session.SessionWrapperManager;
...
import static de.blinkt.openvpnsc.core.ConnectionStatus.LEVEL_CONNECTED;
import static de.blinkt.openvpnsc.core.ConnectionStatus.LEVEL_WAITING_FOR_USER_INPUT;
import static de.blinkt.openvpnsc.core.NetworkSpace.ipAddress;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    public static final String START_SERVICE = "de.blinkt.openvpnsc.START_SERVICE";
    public static final String START_SERVICE_STICKY = "de.blinkt.openvpnsc.START_SERVICE_STICKY";
    public static final String PREPARE_SESSION = "de.blinkt.openvpnsc.PREPARE_SESSION";
    public static final String ALWAYS_SHOW_NOTIFICATION = "de.blinkt.openvpnsc.NOTIFICATION_ALWAYS_VISIBLE";
    public static final String DISCONNECT_VPN = "de.blinkt.openvpnsc.DISCONNECT_VPN";
    public static final String NOTIFICATION_CHANNEL_BG_ID = "openvpn_bg";
    public static final String NOTIFICATION_CHANNEL_NEWSTATUS_ID = "openvpn_newstat";
    public static final String VPNSERVICE_TUN = "vpnservice-tun";
    public static final String PROFILE_UUID = "profile-uuid";
    public final static String ORBOT_PACKAGE_NAME = "org.torproject.android";
    private static final String PAUSE_VPN = "de.blinkt.openvpnsc.PAUSE_VPN";
    private static final String RESUME_VPN = "de.blinkt.openvpnsc.RESUME_VPN";
    ...
    public IBinder onBind(Intent intent) {
        ...
        if (action != null) {
            if (action.equals(START_SERVICE))
                return mBinder;
            else if (action.equals(PREPARE_SESSION)) {
                /* open first session on token to save time for BT */
                String profileUUID = intent.getExtras().getString(PROFILE_UUID);
                VpnProfile profile = ProfileManager.get(getApplicationContext(), profileUUID);
                String serial = profile.mTokenSerial;
                SessionWrapperManager sm = SessionWrapperManager.getInstance();
                if (!sm.hasSessionWrapper(serial)) {
                    Optional<Pkcs11Token> optToken = TokenFinder.getBySerial(serial);
                    if (optToken.isPresent()) {
                        SessionWrapper.Builder builder = new SessionWrapper.Builder(optToken.get());
                        try {
                            sm.prepareSessionWrapper(serial, builder);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
    ...
    synchronized void unregisterDeviceStateReceiver() {
        if (mDeviceStateReceiver != null)
            try {
                ...
                // java.lang.IllegalArgumentException: Receiver not registered: de.blinkt.openvpnsc.NetworkSateReceiver@41a61a10
                // Ignore for now ...
                iae.printStackTrace();
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        tokenManager.addTokenListener(SessionWrapperManager.getInstance());
    ...
    private OpenVPNManagement instantiateOpenVPN3Core() {
        try {
            Class cl = Class.forName("de.blinkt.openvpnsc.core.OpenVPNThreadv3");
    ...
    private void doSendBroadcast(String state, ConnectionStatus level) {
        ...
        vpnstatus.setAction("de.blinkt.openvpnsc.VPN_STATUS");
